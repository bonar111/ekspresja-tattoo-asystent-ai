// src/components/home/HeroRealism.tsx
import { motion } from 'framer-motion';
import { openChat } from '../../lib/openChat';

const HeroRealism = () => {
  const handleOpenChat = (e: React.MouseEvent<HTMLButtonElement>) =>
    openChat({ source: 'hero_cta_realizm', ai_event: 'AIStart', style: 'realism' }, e);

  const handleScrollToWorks = (e: React.MouseEvent<HTMLAnchorElement>) => {
    e.preventDefault();
    const target = document.getElementById('prace');
    if (!target) return;
      ...
      <div className="container relative z-10 px-4 text-center">
        ...
        >
          <h1 id="hero-heading" className="mb-3 text-[28px] leading-tight font-extrabold sm:text-5xl">
            Kolorowy realizm — duże prace
          </h1>

          <p className="mb-5 text-base sm:text-xl">
            Realistyczne projekty w kolorze (portrety, zwierzęta, duże motywy).{' '}
            <strong>Napisz — podeślemy 2 kierunki projektu i najbliższe terminy.</strong>
          </p>
          ...
          <div className="mt-3">
            <a
              href="#prace"
              className="inline-flex items-center gap-1 text-sm opacity-90 hover:opacity-100 underline underline-offset-2"
              onClick={handleScrollToWorks}
            >
              Zobacz prace w realizmie <span aria-hidden>⌄</span>
            </a>
          </div>
